...
writeShellApplication {
  ...
  text = ''
    ...
    if [[ ! -f "$resourcesPath/.complete" ]]; then
      ...
      ln -s \
        "$CHATGPT_RESOURCES_SOURCE/codex" \
        "$CHATGPT_RESOURCES_SOURCE/codex-code-mode-host" \
        "$CHATGPT_RESOURCES_SOURCE/cua_node" \
        "$CHATGPT_RESOURCES_SOURCE/native" \
        "$CHATGPT_RESOURCES_SOURCE/rg" \
        "$stagingPath"
